    <?php
    //provide user details from this page
    include("session.php");
    ...
    function generateDateRange($startDate, $endDate) {
        //This line initializes an empty array called $dates. This array will store the range of dates.
        $dates = array();
        //This line converts the $startDate from a string format (like '2023-01-01') to a Unix timestamp, 
        //which is a numerical representation of the date.
        $currentDate = strtotime($startDate);

        while ($currentDate <= strtotime($endDate)) {
            //Inside the loop, this line converts the current timestamp back to a date string
            // in 'Y-m-d' format (like '2023-01-01') and adds it to the $dates array.
            $dates[] = date('Y-m-d', $currentDate);
            //adding one day to the current timestamp.
            $currentDate = strtotime('+1 day', $currentDate);
        }
//return ['2023-01-01', '2023-01-02', '2023-01-03', '2023-01-04', '2023-01-05'];
        return $dates;
    }
    ...
    <h2 class="try">Expense Report</h2>
        ...
        <div class="col-md-6">
            <!-- sets the default value of the input field to the current date using PHP. -->
            <!-- To provide a calendar for the user to choose a date, you typically use an HTML <input> element of type date -->
    ...
    <div class="form-group row">
        ...
        <button type="submit" class="btn btn-lg btn-block btn-success" style="border-radius: 0%;">Generate Report</button>
                            ...
                            <!-- Display the generated report here -->
                             <!--  if the request method is POST and if the variable $exp_fetched ie sql query is called is set. -->
        ...
        //sno,tableheader,total amount columns created
        echo '<tbody>';

        $count = 1;//s no
        foreach ($periodDates as $periodDate) {//from array of consecutive dates from start to end
            mysqli_data_seek($exp_fetched, 0); // Reset fetch pointer
            //Resets the fetch pointer to the beginning of the result set.
            // This ensures that the data can be fetched again from the start for each period.
            $totalAmount = 0;
            
            //Fetch Rows and Calculate Total Amount on each day
            while ($row = mysqli_fetch_array($exp_fetched)) {
                if ($reportType == 'datewise' && $row['period'] == $periodDate) {
                    //date matched then add expense, else skip date and go to next date
            ...
            if ($totalAmount > 0) { // Only display if there's expense for this period
                echo '<tr>';
                // $count .: This concatenates the current value of the $count variable
                // (which represents the row number) into the cell.
                echo '<td class="text-center">' . $count . '</td>';
                
                if ($reportType == 'datewise') {
                    //datewise, this outputs a table data cell containing the period date as it is.
