...
if (isset($_POST['but_upload'])) { // Check if form is submitted
    $name = $_FILES['file']['name']; // Get the original name of the uploaded file
    $target_dir = "uploads/"; // Define the target directory for uploads
    $target_file = $target_dir . basename($_FILES["file"]["name"]); // Create the full path for the uploaded file with the basename of the file's original name.

    //pathinfo($target_file, PATHINFO_EXTENSION): Extracts the file extension from the target file path.
    //strtolower(): Converts the file extension to lowercase to ensure consistency when checking allowed extensions.
    // Select file type
    $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION)); // Extract and lower-case the file extension

    // Valid file extensions
    $extensions_arr = array("jpg", "jpeg", "png", "gif"); // Define allowed file extensions

    // Check extension
    if (in_array($imageFileType, $extensions_arr)) { // Check if the file extension is allowed
        // Insert record
        $query = "UPDATE users SET profile_path = '$name' WHERE user_id='$userid'"; // Create an SQL query to update the user's profile path
        mysqli_query($con, $query); // Execute the SQL query

        // Upload file
        move_uploaded_file($_FILES['file']['tmp_name'], $target_dir . $name); // Move the uploaded file to the target directory

        header("Refresh: 0"); // Refresh the page immediately
    }
}
